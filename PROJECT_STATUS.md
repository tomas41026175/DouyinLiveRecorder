...
- **孤兒 ffmpeg 造成同一場直播重複錄製（已修，2026-09-24）** — 根因是重啟/部署用的 `taskkill /f /im ...exe` 沒加 `/T`，只殺主程式、留下還在錄的 ffmpeg 孤兒行程，新啟動的 exe 不知道它的存在又開一次新錄製；四處 `taskkill`（`web_ui.py`/`widget.py`/`deploy.bat`/`install_all.bat`）已加 `/T`，`main.py` 也加了「已在 `recording` 集合內就跳過」防呆。詳見 AGENT.md §7。新增 `tools/find_duplicate_sessions.py`（+ `src/dup_session_finder.py`，有單元測試）掃描既有重複檔案；預設純列清單，加 `--quarantine-dir <資料夾> --apply` 可把每組裡較不完整的一份搬去隔離資料夾（只搬不刪，附 manifest 供事後核對/反悔）。
