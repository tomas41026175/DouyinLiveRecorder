...
- **`taskkill /f /im` 不帶 `/T` 造成孤兒 ffmpeg 重複錄製（已修，2026-09-24）**：同一場直播、開始時間差 10~70 秒、各自錄出一整套 `_000`~`_012` 分段、對應 index 的檔案大小幾乎一致——一開始誤以為是「特別關注 60 秒快抓開播」跟主迴圈打架，追下去發現每個 URL 只有一條 thread、輪詢在錄製結束後才進下一輪，同一 process 內結構上不可能重複錄。真正原因：`main.py` 用 `subprocess.Popen(ffmpeg_command, ...)`（`check_subprocess()`）直接開 ffmpeg 子行程，沒設 `CREATE_NEW_PROCESS_GROUP`/`DETACHED_PROCESS`；而 `web_ui.py`（`/api/recorder/restart`）、`widget.py`（tray「Stop recorder」）、`deploy.bat`、`install_all.bat` 四處的 `taskkill /f /im DouyinLiveRecorder.exe` 都**沒加 `/T`**——Windows 上這樣只殺主程式本身，不會連子行程樹一起殺。於是：按「↻ 立即檢查開播」或重新部署時，舊 exe 被殺但它當下在錄的 ffmpeg 變孤兒繼續錄；新 exe 啟動後重讀 `URL_config.ini`，若該主播剛好在直播，新 process 完全不知道舊孤兒行程的存在（`recording = set()` 只存在單一 process 記憶體裡，新 process 重啟就歸零），直接又開一個全新 ffmpeg 錄同一場——`UIUX_SPEC.md` 寫的「按重啟按鈕進行中的錄製會續錄」其實就是這個副作用（沒斷檔是因為孤兒 ffmpeg 真的沒被殺掉）。**修法**：① 上述四處 `taskkill` 全加 `/T`（連子行程樹一起殺，`web_ui.py:2858`/`widget.py:132`/`deploy.bat:42`/`install_all.bat:122`）；② `main.py` 的 `start_record()` 在 `recording.add(record_name)` 前加一道 `if record_name in recording: continue` 防呆（`main.py` `disable_record` 檢查之後），防同一 process 內誤重入（不能防跨 process 孤兒，但屬多一層保護）。**清查既有重複檔**：新增 `tools/find_duplicate_sessions.py`（`src/dup_session_finder.py` 是可測純邏輯：抓「同主播、開始時間差在 `--max-gap` 秒內、重疊 segment index 檔案大小幾乎相同」的 session 配對，並用 union-find 把 A~B、B~C 這種鏈狀關係併成一組）。預設純掃描列清單，不動檔案；加 `--quarantine-dir <資料夾>` 會多印出整理計畫（每組留一份、其餘搬去隔離資料夾），但仍要再加 `--apply` 才真的執行 `shutil.move`（沒有 `--apply` 純預覽）。留哪份用 `--keep largest`（預設，留檔案總大小較大/較完整的那份）或 `--keep earliest`（留最早開始錄的那份，對應孤兒 ffmpeg 是「重啟前那個」的直覺，但如果較早那份自己中途被切斷過會留到殘檔，需要人工核對）。只搬不刪，隔離資料夾裡會附 `manifest_<時間>.json` 記錄每個檔案的原始路徑供事後反悔手動搬回。`plan_quarantine()`/`cluster_duplicate_sessions()` 同樣是無 IO 純函式，`tests/test_dup_session_finder.py` 涵蓋分組/留檔策略。**`start_console.bat` 行為也改了（2026-09-25）**：原本 Step 3 是「recorder 沒在跑才啟動，已經在跑就跳過」，實測發現這個判斷只看 `Get-Process -Name DouyinLiveRecorder` 有沒有行程，不管它是不是殭屍/卡死——排查這次 bug 過程中重啟過幾次後，就出現「跑 `start_console.bat` 卻沒啟動新 recorder」的情況（殭屍行程還在，被誤判成「已經在跑」）。已改成每次啟動都先 `taskkill /F /T /IM DouyinLiveRecorder.exe`（帶 `/T`，孤兒 ffmpeg 也一起清掉）、等它真的消失（最多等 10 秒）再啟動新的，不再嘗試判斷「現有的是不是健康的」。**取捨**：這樣保證每次啟動都是乾淨的新行程，但如果啟動當下剛好有主播在錄，該場錄影會在這一刻被中斷、幾秒後才由新行程重新開始錄（產生兩個檔案，但不會像原本那樣同時重疊錄成真正的重複檔）——這跟 `UIUX_SPEC.md` 原本「重啟不中斷錄影」的說法互相抵觸，是使用者明確要的行為（比起殭屍行程導致啟動失敗，可接受偶爾斷一下錄影）。
